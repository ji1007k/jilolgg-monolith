...
### 6. 로그인 장벽 제거 (Anonymous UX)
- **문제**: 즐겨찾기·리그 순서·경기 알림이 모두 로그인을 요구해, 처음 들어온 사용자가 아무것도 자기 취향대로 바꿀 수 없었다
- **해결**: 기능의 성격에 따라 저장 위치를 나눔
  - **즐겨찾기 / 리그 순서** — 본질적으로 이 브라우저의 취향이므로 비로그인은 `localStorage`. 서버 호출 자체가 없다
  - **경기 알림** — 서버가 "어느 기기가 어느 경기를 구독했는지" 알아야 하므로 브라우저 저장만으로는 불가능. 구독 주체를 `user_id`에서 `owner_key`(`u:<userId>` / `d:<deviceId>`)로 일반화
- **설계 포인트**: 프론트에 저장소 추상화(`utils/userPreferences.js`)를 두어 호출부가 로그인 여부를 모르게 함
- **전환 처리**: 로그인 시 로컬 설정과 계정 설정이 충돌하면 사용자에게 병합 여부를 묻고, 기기에 걸어둔 알림은 FCM 토큰을 매개로 계정에 자동 이관
