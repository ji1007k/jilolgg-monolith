...
## Active
- `development.md`: 주요 개선 이력과 운영 이슈 해결 히스토리
- `swagger-api-guide.md`: Swagger 인증/테스트 절차 및 API 그룹 안내
- `architecture.md`: 아키텍처 설명(레거시 맥락 포함)
...
- `mobile-agent-setup.md`: 모바일에서 지시해 수정·배포하는 구성(@claude 코멘트)
...
## Policy
- 현재 운영/협업에 필요한 문서는 `docs/` 루트에 유지한다.
- 과거 운영 방식/실험 원본/일회성 기록은 `docs/archive/`로 이동해 보관한다.
