...
1. **JWT 무상태 인증 구현**
   ...
   - **CORS 환경에서 쿠키 저장 이슈 해결**:
      - 개발환경: SameSite=None 설정에도 불구하고 강화된 브라우저 보안 정책으로 embedded context(fetch 요청 등)에서 cross-origin 쿠키 저장 실패 → 프록시 미들웨어에서 백엔드 쿠키를 중계하여 same-origin 응답으로 변환 (25.07.28)
